...
public class GameModeTile extends QSTileImpl<BooleanState> {
    ...
    public GameModeTile(QSHost host) {
        ...
        mSetting = new SecureSetting(mContext, mHandler, Secure.GAME_MODE_ENABLE) {
            @Override
    ...
    private void setEnabled(boolean enabled) {
        boolean needsChange = Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.OVERRIDE_POLICY_CONTROL, 0) == 0;
        if (enabled && needsChange) {
            mChanged[0] = true;
            Settings.Global.putInt(mContext.getContentResolver(),
                    Settings.Global.OVERRIDE_POLICY_CONTROL, 1);
        } else if (!enabled && mChanged[0]) {
            mChanged[0] = false;
            Settings.Global.putInt(mContext.getContentResolver(),
        ...
        Settings.Secure.putInt(mContext.getContentResolver(),
                Settings.Secure.SYSTEM_GAME_MODE_ENABLE,
                enabled ? 1 : 0);
    }
